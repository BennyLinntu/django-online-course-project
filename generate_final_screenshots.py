...
def create_admin_screenshot():
    ...
    draw.text((40, 30), "Django administration",
              fill='#FFD700', font=header_font)
    draw.text((40, 75), "Welcome, admin. View site | Change password | Log out",
    ...
    draw.rectangle([(30, y), (1250, y + 50)], fill='#2C5282',
                   outline='#3B82F6', width=2)
    draw.text((50, y + 12), "AUTHENTICATION AND AUTHORIZATION",
              fill='white', font=section_font)
    y += 70
    ...
    draw.rectangle([(30, y), (1250, y + 50)], fill='#2C5282',
                   outline='#3B82F6', width=2)
    draw.text((50, y + 12), "ONLINECOURSE_APP",
              fill='white', font=section_font)
    y += 70

    app_items = ["Choices", "Courses", "Enrollments",
...
def create_exam_result_screenshot():
    ...
    draw.text(
        (40, 25), f"Exam Result for {course.name}", fill='white', font=title_font)
    y = 120

    # Success message
    draw.rectangle([(30, y), (1250, y + 60)], fill='#D1FAE5',
                   outline='#10B981', width=2)
    draw.text((50, y + 15), "Congratulations! You have passed the exam.",
              fill='#065F46', font=normal_font)
    y += 80
    ...
    # Correct answers
    draw.text(
        (40, y), f"Correct Answers: 1 out of {questions.count()}", fill='#1F2937', font=normal_font)
    y += 40
    ...
    for question in questions:
        draw.rectangle([(30, y), (1250, y + 80)], fill='white',
                       outline='#D1D5DB', width=2)
        draw.text(
            (50, y + 10), f"Question {question_num}: {question.question_text}", fill='#1F2937', font=normal_font)
        draw.text((50, y + 35), "Your answer: A programming language",
                  fill='#4B5563', font=normal_font)
