...
def test_workflow():
    ...
    for question in questions:
        ...
        for choice in choices:
            print(f"    - Choice: {choice.choice_text} (Correct: {choice.is_correct})")
    ...
    if response.status_code == 200:
        print(f"✓ course_details view accessible (status: {response.status_code})")
    else:
    ...
    if first_question:
        correct_choice = first_question.choice_set.filter(is_correct=True).first()
        if correct_choice:
            post_data = {f'question_{first_question.id}': correct_choice.id}
            response = client.post(f'/onlinecourse/submit/{course.id}/', post_data)
            if response.status_code == 302:  # Redirect expected
                print(f"✓ submit view works (status: {response.status_code})")
                print(f"  Session exam_result: {response.wsgi_request.session.get('exam_result', 'Not found')}")
            else:
    ...
    if response.status_code == 200:
        print(f"✓ show_exam_result view accessible (status: {response.status_code})")
    else:
        print(f"✗ show_exam_result view failed (status: {response.status_code})")
